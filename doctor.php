<?php
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT email FROM doctor WHERE email = '$email' and password = '$password'";
    ...
    if($count == 1) {
         ...
         $_SESSION['login_userd']= $email;
         echo $_SESSION['login_userd'];
         //if(isset($_SESSION['login-user']))
        header("location: \login\docpro.php");
        echo "success";
      }else {
...
<head>
  <meta charset="UTF-8">
  <title>Healthify Doctor's Login</title>
  ...
  <h1 style="color:#00ff00">Doctor's Login</h1>
</div>
</head>
